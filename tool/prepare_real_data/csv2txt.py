...
# -*- coding: utf-8 -*-
"""
Remap the ABlueSky edge list CSV (follower, followee, date) into consecutive IDs 0..N-1,
and output in the specified format:
The 1st line is the number of nodes; each following line is one edge with u < v (normalized undirected).
Note: this script does NOT deduplicate edges. If the same undirected edge appears multiple times 
in the input, it will also appear multiple times in the output.
If deduplication is needed, you can use external sort -u after output (see previous examples).
"""
...
# ---------- Hard-coded paths ----------
IN_PATH  = "./data/real-large-graph/deidentified_follows_edgelist.csv"
OUT_PATH = "./data/real-large-graph/bluesky.txt"
...
def pass1_scan_max_and_lines():
    """First pass: scan to get max_id and total lines, 
    in order to allocate mapping array and report progress later."""
    ...
    with open(IN_PATH, "r", buffering=BUFFER) as fin:
        for line in fin:
            ...
            if len(parts) < 2:
                continue
            try:
                u = int(parts[0]); v = int(parts[1])  # discard the 3rd column (date)
            except ValueError:
                continue
...
def pass2_build_mapping(max_id, total_lines):
    """
    Second pass: build old_id -> new_id mapping to make node IDs consecutive 0..N-1.
    Use 4-byte int array, initialized with -1. Memory ~ (max_id+1)*4 bytes.
    """
    print("[Pass2] Allocating mapping array and building ID remapping (old -> new 0..N-1) ...")
    st = time.time()
    try:
        mapping = array('i', [-1]) * (max_id + 1)  # sentinel -1
    except MemoryError:
        print("❌ Not enough memory: cannot allocate mapping array", file=sys.stderr); sys.exit(1)
    ...
    print(f"[Pass2] Done: unique_nodes={next_id:,}, elapsed {time.time()-st:.1f}s")
    return mapping, next_id

def pass3_write_output(mapping, num_nodes, total_lines):
    """
    Third pass: write final output file.
    ...
    Note: no deduplication. If needed, use external deduplication after output.
    """
    ...
    with open(IN_PATH, "r", buffering=BUFFER) as fin, \
        ...
        for line in fin:
            ...
            if len(parts) < 2:
                continue
            try:
                u0 = int(parts[0]); v0 = int(parts[1])  # discard the date column
            except ValueError:
                continue
